...
func exe(user *User) {
	...
	println(fmt.Sprintf("rules num:%d, load rules cost time:%d ns", len(ruleBuilder.Kc.RuleEntities), end1.Sub(start1).String()))
	...
	println(fmt.Sprintf("execute rule cost %d ns", end.Sub(start).String()))
